...
namespace FronkonGames.SpiceUp.Drunk.Editor
{
  ...
  public abstract partial class Inspector : UnityEditor.Editor
  {
    ...
    public static GUIContent NewGUIContent(string label, string name, string tooltip) => new GUIContent(string.IsNullOrEmpty(label) == false ? label : HumanizeName(name), tooltip);
  }
}
